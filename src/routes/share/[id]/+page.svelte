...
<button
  onclick={back}
  aria-label="Back"
  class="fixed top-[calc(env(safe-area-inset-top)+1rem)] left-4 z-20 flex h-10 w-10 items-center justify-center rounded-full bg-white/80 text-lg text-stone-600 shadow-sm ring-1 ring-stone-200 backdrop-blur-xl hover:text-stone-900"
>
  ←
</button>

{#await quotePromise then quote}
  {#if quote}
    <h1 class="mb-4 text-center text-xl font-bold text-stone-900">
      Share this quote
    </h1>
    ...
    <div
      class="fixed inset-x-0 bottom-0 z-10 flex justify-center px-4 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] lg:hidden"
    >
      <button
        onclick={onShare}
        disabled={busy}
